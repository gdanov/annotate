(ns annotate.types
  ...
  #?(:cljs (:require-macros [annotate.types :refer
                             [defprimitive IFn Pred defcan augment-constructor
                              augment-type augment-primitive]]))
...
(defrecord IFnVType [arglists]
  ;; checked IFnType. arglists is in the format that fnv uses
  Typeable
  (display-type [this] '(FUNCTION ETC))
  (valid-type? [this] true)
  (check [this that]
    (println "!!--!!" (::contract (meta that)))))
